...
# @Filename: step16_multi_processing_step2
# @Date: 2019-01-08
# @Author: Mark Wang
...
"""
python3 -m generate_vars_20180910.step16_multi_processing_step2
"""
...
if __name__ == '__main__':
    data_df = pd.read_pickle(os.path.join(const.TEMP_PATH, '20180910_merged_psm_data_file_rcon5571.pkl'))
    ...
    distance_tmp_save_path = os.path.join(const.TEMP_PATH, '20190109_distance_match')
    constructed_tmp_save_path = os.path.join(const.TEMP_PATH, '20190109_constructed_distance_variables')
    ...
    all_distance_df.to_pickle(os.path.join(const.TEMP_PATH, '20190109_constructed_distance_variables.pkl'))

    merged_previous_info_df = data_df.merge(all_distance_df,
                                            on=[const.YEAR_MERGE, const.QUARTER, 'Target_id', 'Acquirer_id'],
                                            how='left')
    merged_previous_info_df.to_pickle(os.path.join(const.TEMP_PATH, '20190109_psm_add_distance_vars_rcon5571.pkl'))
    merged_previous_info_df.to_csv(os.path.join(const.RESULT_PATH, '20190109_psm_add_distance_vars_rcon5571.csv'),
                                   index=str)

...
if __name__ == '__main__':
    ...
    df = df.rename(index=str, columns={'Year': const.YEAR_MERGE, 'DateAnnounced': const.ANNOUNCED_DATE,
    ...
    df = df.dropna(subset=[const.ANNOUNCED_DATE, const.YEAR_MERGE], how='all')
    df[const.YEAR_MERGE] = df[const.YEAR_MERGE].fillna(
        df[df[const.YEAR_MERGE].isnull()][const.ANNOUNCED_DATE].apply(lambda x: x.year)).apply(int)
    ...
    acq_df = df[[const.YEAR_MERGE, const.ACQUIRER_MVE, const.ACQUIRER_NI, const.ACQUIRER_TA, const.ACQUIRER_CUSIP,
                 const.ACQUIRER_TICKER, const.ACQUIRER_LT, const.ACQUIRER_ROA, const.ACQUIRER_TOBINQ]]
    tar_df = df[[const.YEAR_MERGE, const.TARGET_CUSIP, const.TARGET_TICKER, const.TARGET_NI, const.TARGET_TA]]
    ...
    cleaned_df = merged_df.groupby([const.YEAR_MERGE, const.COMPUSTAT_CUSIP]).apply(get_information).dropna(
